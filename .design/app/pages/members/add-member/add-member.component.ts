...
export class AddMemberComponent extends NgWizardStep implements OnInit {
	constructor(public __elementSelectionService:ElementSelectionService,private __componentInspectorService:ComponentInspectorService) { this.__componentInspectorService.getComp(this); }pk: string = 'id';
	title: string;
	...
	ngOnInit() {
	    ...
	    this.memberTypeService.getAll().subscribe(data=>{
	    	...
	    	let obj = this.memberTypes.filter(obj => obj.TypeID ===this.dataService.formValues.TypeID);
	    ...
	    let username = Functions.randomNumber(4) + '/' + (today.getMonth()+1) + '/' + today.getFullYear();
