...
export class NotificationService {
  ...
  send(data: any): Observable<any> {
    return this.http.post(API_BASE + '-send', data,httpOptions);
  }
  deleteAll(): Observable<any> {
    return this.http.delete(API_BASE,httpOptions);
  }

  findByMessageTitle(MessageTitle: any): Observable<any[]> {
    return this.http.get<any[]>(`${API_BASE}?MessageTitle=${MessageTitle}`,httpOptions);
  }
}
